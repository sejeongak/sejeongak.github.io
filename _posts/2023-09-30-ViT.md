...
>
>    CNN의 inductive bias  
>    1. 공간적인 위치 정보 활용: CNN은 입력 이미지의 공간 구조를 활용하는데, 이는 합성곱 연산을 통해 입력 데이터의 특정 위치와 주변 정보를 고려하여 feature map을 생성하는 방식으로 나타납니다.
>
>    2. shared weight, feature map: CNN은 합성곱 연산에서 가중치를 공유하여 모델 파라미터 수를 줄이고, 데이터에서 다양한 위치에서 유사한 패턴을 감지하는데 도움이 됩니다.
>
>    3. 계층적 특성 추출: CNN 아키텍처는 보통 여러 계층으로 구성되며, 하위 계층은 간단한 특징을 추출하고 상위 계층은 더 추상적인 특징을 학습합니다.
>
>    4. 풀링 계층: CNN에는 풀링 계층이 포함되어 있어 공간 해상도를 줄이고 계산 효율성을 향상시킵니다. 이것은 이미지 스케일 불변성(scale invariance)과 이동 불변성(shift invariance)을 제공하는 데 도움을 줍니다.
>
